...
Graph.prototype.bfsCallback = function(event) {
	if($("#bfsBtn").hasClass("disabled") || $("#bfsVal button").text().trim() == "vertex") {
		return;
	}
		...
		if (this.seqSearch(visit, VERTICES_SIZE, currentVertex) == 0) {
			...
			for (let i = 0; i < VERTICES_SIZE; i++) {
				if (adj[currentVertex + "-" + i] == 1) {
					...
					this.cmd("SetText", this.visitedVertices[i], "1");
					this.cmd("Step");
					this.cmd("Step");
					if (!visited[i]) {
			...
			if (fp["next"] != null && flag && bfs[nextElmt] == undefined) {
				...
				this.cmd("BFSStep");
				var text = "Here, there is no adjacent vertices for vertex <y> " + nextElmt + "</y>, " 
				...
				this.cmd("BFSTooltipPos", VERTICES_FIXID_X_POS[nextElmt] + 30, VERTICES_FIXID_Y_POS[nextElmt] - 20);
				this.cmd("BFSStep");
				var text = "Find all the adjacent vertices for vertex <y>" + nextElmt +  "</y>. <br/>They are : "
