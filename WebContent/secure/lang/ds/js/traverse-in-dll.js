		...
		$('.introjs-helperLayer').one('transitionend', function() {
			...
			case "algorithmStepsDiv" :
				...
				case 'Step1':
					...
					appendSteps(1);
					var text = '<ul><li>Check the condition <bgb>first</bgb> is equal to <g>NULL</g> or not.</li>'
				...
				case "Step2" :
					...
					var text = '<ul><li>If it is <brown>FALSE</brown>, then take one temporary node <g>last</g> and initialize with the <bgb>first</bgb>'
								...
								+ ' <li>If it is <g>not NULL</g> then print the <g>data</g> field of <g>last</g> and assign <g>next</g> field of '
...
function ifTempValueIsNull() {
	$('.user-btn, #btn').remove();
	$('.introjs-tooltiptext ul li:last').after('<li></li>');
	text = 'Check if the <bgb>first</bgb> value is <y>NULL</y>.';
	typing('.introjs-tooltiptext ul li:last', text, function() {
...
function ifTempValueIsNullAnimation() {
	...
	if ($('#firstVal').text() == "NULL") {
		...
		text = 'Here, the condition is evaluates to <y>FALSE</y>, so repeat the loop until <y>last</y> is <y>equal</y> to <y>NULL</y>.<br/>'
...
function createDynamicNodes(nodeNum) {	//node
	...
	$('#prev' + nodeNum + ', #data' + nodeNum + ', #next' + nodeNum).addClass('position ct-fonts');
	...
	var toolTopText = "This is an pointer type to hold the previous node's address";
