...
function lineTesting() {
	...
	svgLineRightAndLeft('#animationDiv1', '#box11', '#box12', 'line2', 'right', 'top', 'top', true);
	svgLineTopAndBottom('#animationDiv', '#box1', '#box3', 'line11', 'bottom');
	//svgLineRightAndLeft('#box1', '#box2', 'line1', 'black', 'arrow', 'right', 'top', 'top', true);
...
function svgAppend(parentId, svgId, markerId, markerColor, lineColor) {
	if (typeof parentId === "undefined" || parentId.trim() == '') {
		...
		var ctLineTestingArr = []
		ctLineTestingArr.push({"parentId" : parentId, "svgId" : "#" + svgId, "markerId": markerId, "lineColor": lineColor});
		ctLineTesting[parentId] = ctLineTestingArr; 
...
function svgMarkerAppend(parentId, markerColor) {
	...
	marker.setAttribute('id', ctLineTesting[parentId][0].markerId);
	...
	$(ctLineTesting[parentId][0].svgId).append(marker);
	var path = document.createElementNS("http://www.w3.org/2000/svg", 'path');
	path.setAttribute("d", "M0,0 L5,2.5 L0,5 Z");
	$('#' + ctLineTesting[parentId][0].markerId).append(path);
}
...
function svgLineAppend(parentId, svgLineId, x1, y1, x2, y2, styleFlag) {
	...
	line.style.stroke = ctLineTesting[parentId][0].lineColor;
	line.setAttribute("x1", x1);
	line.setAttribute("y1", y1);
	line.setAttribute("x2", x2);
	line.setAttribute("y2", y2);
	line.style.markerEnd = 'url("#' + ctLineTesting[parentId][0].markerId + '")';
	$(ctLineTesting[parentId][0].svgId).append(line);
}
